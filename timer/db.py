...
def transaction(is_commit=False):
    def _transaction(func):
        ...
        def __transaction(self, *args, **kwargs):
            try:
                self.mysql_conn = mysql_pool.connection()
                ...
                if is_commit and self.mysql_conn:
                    self.mysql_conn.rollback()
                # 记录日志
                logger.error(traceback.format_exc())
            finally:
                if self.mysql_conn and self.mysql_cur:
...
class MysqlDbUtil(object):
    ...
    def update(self, data, table_name=None):
        ...
        for k, v in data.iteritems():
            ...
            if '_ptr_id' in k:
                pk_field = k
        condition = data[pk_field]
        if isinstance(condition, list):
            sql = sql[:-1] + " WHERE `{}` in ({})".format(
                pk_field, ",".join(condition))
        else:
